...
- получает attachment-ы страницы;
- скачивает S2T-файлы;
...
- находит S2T-кандидаты в attachment-ах, ссылках, таблицах и дочерних страницах;
...
- через Confluence API запрашиваются attachments;
...
А. Attachments через API

- `ConfluenceClient.get_attachments(page_id)` получает список файлов страницы;
- для каждого attachment доступны `id`, `title`, `version`, `fileSize`, `mediaType`,
  `downloadUrl`;
...
- если ссылка найдена в HTML, ресурс обогащается metadata соответствующего attachment-а из API.

В. Дочерние страницы S2T

- если дочерняя страница похожа на S2T-раздел, парсер проверяет ее HTML/storage body;
- attachments дочерней страницы также запрашиваются через API;
- найденные ссылки и таблицы обогащаются metadata attachment-ов дочерней страницы.
...
| Файл найден, но не скачивается | У ресурса нет `download_url` или ссылка не является attachment-ом. | Лучше прикрепить S2T как attachment Confluence, а не внешнюю ссылку. |
