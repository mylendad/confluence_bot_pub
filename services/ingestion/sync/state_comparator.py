...
from shared.storage.s2t_state_repository import S2TState
    ...
    def compare(
        ...
        if old_metadata == metadata:
            ...
            return MetadataDecision(
                changed=False, reasons=["metadata hash changed but content same"]
            )
        ...
        for key, value in metadata.items():
            ...
            if old_val != value:
                ...
                logger.info(
                    "Metadata mismatch for '%s': key='%s' old=%r new=%r",
                    metadata.get("datamart_name"),
                    key,
                    old_val,
                    value,
                )
        ...
        for key in old_metadata:
            if key not in metadata:
                ...
                logger.info(
                    "Metadata key removed for '%s': key='%s'", metadata.get("datamart_name"), key
                )
